...
class AceStreamPlayer(xbmc.Player):
    ...
    def loop(self):
        ...
        if plugin.get_setting("keep_files", bool):
            ...
            engine_params["dlpath"] = xbmc.validatePath(xbmc.translatePath(plugin.get_setting("dlpath"))) or None

            if "dlpath" in engine_params and "://" in engine_params["dlpath"]:
        ...
        with closing(AceEngine(**engine_params)) as engine:
            ...
            with closing(SafeDialogProgress(delay_create=0)) as dialog:
                ...
                while (time.time() - start) < ACESTREAM_CONNECTION_TIMEOUT:
                    ...
                    dialog.update(0, self.display_name, "Попытка соединения с AceStream (%s)" % n)
                    if not connected:
                ...
                dialog.update(0, self.display_name, "Соединение установлено. Запуск загрузки данных...")
                ...
                while not has_resolved:
                    ...
                    if status["error"]:
                        dialog.update(int(status["progress"]), *[self.display_name, STATE_STRS[status["status"]], status["error"]])
...
class AceEngine():
    ...
    def _track_sink_event(self, event, params):
        ...
        if event == "STATUS":
            ...
            parts = params.split(" ")
            self.link = parts[0].replace("127.0.0.1", self.host)
            ...
            parts = params.split(" ")
            try: 
                self.version = parts[0].split("=")[1]
            except: 
                self.version = "1.0.6"
            try: 
                self.key = parts[1].split("=")[1]
            except: 
                self.key = None
