...
class AceStreamPlayer(xbmc.Player):
    ...
    def loop(self):
        ...
        engine_params = {
            ...
            "host"     : plugin.get_setting("ace_host", DEFAULT_ACESTREAM_HOST),
            "port"     : plugin.get_setting("ace_port", DEFAULT_ACESTREAM_PORT),
...
class AceEngine():
    ...
    def _get_ace_port(self):
        aceport = self.port
        if _is_local() and PLATFORM["os"] == "windows":
            try:
                import _winreg
                key = _winreg.OpenKey(_winreg.HKEY_CURRENT_USER, "Software\\AceStream")
    ...
    def _track_sink_event(self, event, params):
        ...
        if event == "STATUS":
            ...
            parts = params.split(" ")
            self.link = parts[0].replace(DEFAULT_ACESTREAM_PORT, self.host)
