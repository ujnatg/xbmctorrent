...
def url_get(url, params={}, headers={}, post = None):
    ...
    if not _cookiejar:
        ...
        sid_file = os.path.join(xbmc.translatePath('special://temp/'), 'plugin.video.xbmctorrent.t3d.cookies.dat') #
...
def torrents3d_page(section, page):
    ...
    for node in nodes:
        ...
        text = "[COLOR FFFFFFCC][%s][/COLOR] %s (%s S:%s)" % (_torrents3d_cleantitle(article_href.text) + (size.replace("&nbsp;", " "), seeds))
        yield {
            "label": text,
