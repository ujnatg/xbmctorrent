...
def url_get(url, params={}, headers={}, post = None):
    ...
    if not STATIC_VARS["cookie"]:
        ...
        sid_file = os.path.join(xbmc.translatePath('special://temp/'), 'plugin.video.xbmctorrent.LostFilm.cookies.dat') #
            ...
            if ((not "-" in episode and int(episode) == 99) or ("." in season and "-" in episode)) and ace_supported():
                ...
                for index, item in enumerate(metadata["info"]["files"]):
                    ...
                    yield _lostfilm_updateitem_from_info({
                        ...
                        "stream_info" : {
                            "video"   : get_quality_from_name(file_name)
                        }
                    }, show_info)
                _lostfilm_close_dbase()
            else:
                plugin.redirect(plugin.url_for("play", uri=generate_magnet(metadata, uenc(selected.get_text()))))
...
def _lostfilm_login():
    ...
    login_url = urljoin(LOGIN_URL, "login.php")
    ...
    html = url_get(action_url, post=values, headers=HEADERS)
...
def _lostfilm_dbase():
    if not STATIC_VARS["connection"]:
        ...
        STATIC_VARS["connection"] = connection

    return STATIC_VARS["connection"]

def _lostfilm_close_dbase():
    if STATIC_VARS["connection"]:
        STATIC_VARS["connection"].close()
