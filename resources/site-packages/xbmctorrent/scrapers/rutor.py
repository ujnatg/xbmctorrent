...
CATEGORIES = (
    ...
    ("5",  u"Наши фильмы", "movied", "kinopoisk"),
...
def rutor_page(catind, page, query = None):
    ...
    from xbmctorrent.utils import url_get, terminating, SafeDialogProgress
    ...
    with closing(SafeDialogProgress(delay_close=0)) as dialog:
        ...
        for node in nodes:
            ...
            magnet_node, title_node = cells[1].findAll("a")
            size = cells[len(cells)-2].text
            tid = title_node["href"][9:title_node["href"].rfind(u"/")]
        ...
        def _get_torrent_info(item):
            ...
            meta.setdefault("stream_info", {}).update({
                "video": get_quality_from_name(meta['label']),
            })
            return meta
...
def rutor_details(catind, tid):
    ...
    from bs4 import BeautifulSoup, SoupStrainer
    from urlparse import urljoin
    from xbmctorrent.utils import url_get, get_quality_from_name
    from xbmctorrent.search import scrapers as search
    ...
    if len(div_index) > 0:
        ...
        for node in nodes:
            ...
            size = cells[len(cells)-2].text
            title = _rutor_cleantitle(title_node.text)
            tid = title_node["href"][9:title_node["href"].rfind("/")]
            label = "%s | %s (S:%s P:%s)" % (title, size, seeds, peers)

            item = scrapers.default({ "label" : label })
            item["path"] = plugin.url_for("rutor_play", tid=tid)
            ...
            item.setdefault("stream_info", {}).update({
                "video": get_quality_from_name(item['label']),
            })
...
def rutor_play(tid):
    ...
    from xbmctorrent.utils import first, url_get, SafeDialogProgress
    ...
    with closing(SafeDialogProgress(delay_close=0)) as dialog:
        ...
        if "files" in metadata["info"] and ace_supported():
            ...
            if len(items) == 1:
                ...
                if plugin.get_setting("force_ace", bool):
            ...
            if plugin.get_setting("force_ace", bool) and ace_supported():
                item["path"] = plugin.url_for("torrent_play", url=torrent_url, index=0, name=name)
            else:
                item["path"] = plugin.url_for("play", uri=generate_magnet(metadata, name))
