...
class TorrentPlayer(xbmc.Player):
    def init(self, uri):
        ...
        self.torrent2http_options = {
            "uri": uri,
            "dlpath": xbmc.validatePath(xbmc.translatePath(plugin.get_setting("dlpath") or "special://temp/" + plugin.id)),
                ...
                while not has_resolved:
                    ...
                    if status["state"] >= 3 and not has_resolved: # Downloading?
                        ...
                        if percent_complete >= MIN_COMPLETED_PIECES:
                            file_name = biggest_file["name"].encode('utf-8')
                            plugin.log.info("Resolving to http://%s/files/%s" % (t2h_instance.bind_address, file_name))
                            has_resolved = True
                            url_name = "/".join(map(urllib.quote, file_name.split("/")))
